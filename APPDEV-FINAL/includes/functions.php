...
function product_sizes(string $sizes): array
{
    $parsed = preg_split('/\s*,\s*/', trim($sizes), -1, PREG_SPLIT_NO_EMPTY) ?: [];
...
function get_cart_items(): array
{
    [$where, $params] = cart_owner_clause();
    $stmt = db()->prepare("
        SELECT cart_items.*, products.name, products.price, products.stock, products.image_url, products.color, products.size AS available_sizes
        FROM cart_items
...
function merge_guest_cart_into_user(int $userId): void
{
    ...
    $pdo = db();
    $items = $pdo->prepare('SELECT product_id, selected_size, quantity FROM cart_items WHERE session_id = ?');
    $items->execute([$sessionId]);

    foreach ($items->fetchAll() as $item) {
        $existing = $pdo->prepare('SELECT id, quantity FROM cart_items WHERE user_id = ? AND product_id = ? AND selected_size = ?');
        $existing->execute([$userId, $item['product_id'], $item['selected_size']]);
        $row = $existing->fetch();
        ...
        if ($row) {
            ...
            $pdo->prepare('UPDATE cart_items SET user_id = ?, session_id = NULL, updated_at = CURRENT_TIMESTAMP WHERE session_id = ? AND product_id = ? AND selected_size = ?')
                ->execute([$userId, $sessionId, $item['product_id'], $item['selected_size']]);
        }
    }
